...
function subString(str) {
    const middleData = str.substring(6, str.length - 4);
    return middleData
}
    ...
    try {
        for (let i = 0; i < data.length; i++) {
            ...
            if (crcResult === crc_rs) {
                ...
                const subResponse = subString(hexData);
                currentSubData.push(subResponse);
            } else {
...
async function sendMessage1(data, timeout = 5000) { // 增加超时 默认5秒 超时的时间可以单独设置
    return new Promise((resolve, reject) => {
        const address = data[0];
        const funcCode = data[1];
        const startAddress = data[2];
        ...
        const timeoutId = setTimeout(() => { // 超时情况
...
async function processData(command) {
    ...
    try {
        ...
        if (crcResult === crc_rs && Buffer.compare(crc, rs) === 0) { // 判断crc校验以及发送和接收的数据是否一致
            console.log('数据修改成功!');
        } else {
